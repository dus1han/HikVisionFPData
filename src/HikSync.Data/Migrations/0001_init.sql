-- LOCAL edge database schema (capture + offline buffer).
-- Central (API-owned) Postgres has its own table; not created here.

-- Device pairs: one location, an IN (enrollment master) and an OUT terminal.
-- Per-role credential columns so IN and OUT may differ. Passwords are plain text.
...
-- Captured attendance events. Identity is the reset-immune composite, surfaced as idempotency_key.
...
-- Optional per-user delta tracking for sync (hash of user record + templates).
